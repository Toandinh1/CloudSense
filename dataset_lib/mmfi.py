...
def decode_config(config):
    ...
    if config["split_to_use"] == "random_split":
        ...
        subjects_train = config["cross_subject_split"]["train_dataset"][
            "subjects"
        ]
    ...
    def load_database(self):
        for scene in sorted(os.listdir(self.data_root)):
            ...
            for subject in sorted(
                os.listdir(os.path.join(self.data_root, scene))
            ):
                ...
                for action in sorted(
                    os.listdir(os.path.join(self.data_root, scene, subject))
                ):
                    ...
                    for modality in [
                        ...
                        if (
                            subject
                            not in self.modalities[modality][scene].keys()
                        ):
                            self.modalities[modality][scene][subject] = {}
                        if (
                            action
                            not in self.modalities[modality][scene][
                                subject
                            ].keys()
                        ):
                            self.modalities[modality][scene][subject][
...
class MMFi_Dataset(Dataset):
            ...
            for action in actions:
                if self.data_unit == "sequence":
                    ...
                    for mod in self.modality:
                        data_dict[mod + "_path"] = os.path.join(
                            self.data_base.data_root,
                            self.get_scene(subject),
                            subject,
                            action,
                    ...
                    for idx in range(frame_num):
                        ...
                        for mod in self.modality:
                            data_dict[mod + "_path"] = os.path.join(
                                ...
                                action,
                                mod,
                                "frame{:03d}".format(idx + 1)
                                + self.get_data_type(mod),
                            )
    ...
    def read_dir(self, dir):
        ...
        if mod in [
            ...
            "infra2",
            "rgb",
        ]:  # rgb, infra1 and infra2 are 2D keypoints
            ...
            for csi_mat in sorted(glob.glob(os.path.join(dir, "frame*.mat"))):
                ...
                data_mat = (data_mat - np.min(data_mat)) / (
                    np.max(data_mat) - np.min(data_mat)
                )
...
def make_dataset(dataset_root, config):
    ...
    val_dataset = MMFi_Dataset(
        database, config["data_unit"], **config_dataset["val_dataset"]
    )
...
def collate_fn_padd(batch):
    ...
    batch_data = {
        ...
        "idx": (
            [sample["idx"] for sample in batch] if "idx" in batch[0] else None
        ),
    }
...
def make_dataloader(
    dataset, is_training, generator, batch_size, collate_fn_padd=collate_fn_padd
):
    loader = DataLoader(
        dataset,
