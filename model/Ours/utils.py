...
def apply_bit_error(tensor: torch.Tensor, error_rate: float) -> torch.Tensor:
    """
    Mô phỏng lỗi bit bằng cách thay đổi ngẫu nhiên các bit trong tensor.
    
    :param tensor: Tensor đầu vào có kích thước 14336 kiểu torch.Int64.
    ...
    # Xác định số lượng bit cần thay đổi
    num_bits_to_modify = int(error_rate * modified_tensor.size(0))
    
    # Chọn ngẫu nhiên các chỉ số để thay đổi
    indices_to_modify = torch.randint(0, modified_tensor.size(0), (num_bits_to_modify,))
...
def apply_bit_loss(tensor: torch.Tensor, loss_rate: float) -> torch.Tensor:
    ...
    indices_to_keep = torch.tensor([i for i in range(tensor.size(0)) if i not in indices_to_remove.tolist()])
