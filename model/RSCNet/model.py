...
import numpy as np
import torch
from .module import Decoder, Encoder, HumanPoseEstimator, RecurrentBlock
...
class RSCNet(nn.Module):
    def __init__(self, config=True, compression_rate=1, output_hpe=36):
        ...
        self.recurrent_block = RecurrentBlock(
            self.embedding_size, self.config["recurrent_block"]
        )
    ...
    def forward(self, x, check_compression_rate=False):
        ...
        new_x = new_x.view(
            batch_size * self.sequence_length, self.num_frames, 3, 10
        )
...
if __name__ == "__main__":
    ...
    inputs = torch.rand(
        size=(1, 3, 114, 10), dtype=torch.float32
    )  # Batch size of 1
