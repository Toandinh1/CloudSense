...
theory_real_compressrate_dict = {"1": 1, "36": 36, "86": 85, "1710": 1700}


def main_RSCNet(data_loader, model_config, device, all_checkpoint_folder):
    for k, v in theory_real_compressrate_dict.items():
        ...
        for epoch in tqdm(range(model_config["epoch"])):
            ...
            for idx, data in enumerate(data_loader["train"]):
                ...
                reconstructed_csi, pred_xy_keypoint = model(
                    csi_data, check_compression_rate
                )
            ...
            with torch.no_grad():
                ...
                for idx, data in enumerate(data_loader["valid"]):
                    ...
                    loss = (
                        model_config["lambda"] * recontruction_loss + hpe_loss
                    )
                    ...
                    # print(pred_xy_keypoint.shape, xy_keypoint.shape)
                    # print(pred_xy_keypoint, xy_keypoint.shape)
        ...
        model = torch.load(
            os.path.join(checkpoint_folder, "last.pt"), weights_only=False
        )
        metric = []
        avg_nmse = []
        torch.cuda.empty_cache()
